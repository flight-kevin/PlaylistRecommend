class PlaylistNode():
    ...
    def updateViewCount(self, pv_view_count):
        self.view_count = pv_view_count
...
class PlaylistTree():
    ...
    def getNodeByLevelName(self, pv_parent_node_id, pv_level_id, pv_node_name):
...
ary_PlaylistTree.addPlaylistNode(PlaylistNode(0, 0, "/", 0, 0))  # root node of ary_PlaylistTree.playlist
for NewPlaylist in NewPlaylistTable:
    ...
    while gv_level_id < len(NewPlaylist):
        ...
        gv_level_id += 1
        cur_node = ary_PlaylistTree.getNodeByLevelName(gv_parent_node_id, gv_level_id, gv_node_name)
        if len(cur_node) > 0:
            updateResult = ary_PlaylistTree.updateNodeByID(cur_node[0].node_id, cur_node[0].view_count + 1)
            gv_parent_node_id = cur_node[0].node_id
        else:
            gv_node_id = max(node.node_id for node in ary_PlaylistTree.playlist) + 1
            ary_PlaylistTree.addPlaylistNode(PlaylistNode(gv_node_id, gv_level_id, gv_node_name, 1, gv_parent_node_id))
...
for TestNode in TestPlaylist:
    #ParentNode = ary_PlaylistTree.getNodeByID(gv_parent_node_id)
    parent_node = ary_PlaylistTree.getNodeByLevelName(gv_parent_node_id, gv_level_id, TestNode)
...
print("\nRecommendation list:")
print("%s\t%s\t%s\t%s\t%s" % ("node_id", "level_id", "parent_node_id", "node_name", "view_count"))
for member in ary_PlaylistTree.getChildrenNodesByID(gv_parent_node_id):
    print("%2d\t%2d\t%2d\t%s\t%3d" % (
        member.node_id, member.level_id, member.parent_node_id, member.node_name, member.view_count))